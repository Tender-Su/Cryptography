...
class Min(QtWidgets.QMainWindow,Ui_MainWindow):
    ...
    def func1(self):
        ...
        for i in range(self.block-1):
            for j in range(4):
                for k in range(4):
                    self.m[i][j] += ord(self.char_m[16 * i + 4 * j + k]) << (24 - 8 * k)
        for j in range(4):
            for k in range(4):
                if (4 * j + k < (len(self.char_m)-1) % 16+ 1):
                    self.m[self.block - 1][j] += ord(self.char_m[16 * (self.block - 1) + 4 * j + k]) << (24 - 8 * k)
    ...
    def Crypt(self, In,  flag):
        ...
        for j in range(32):
            if ( flag ==1 ):
                ...
                state[j + 4] = state[j] ^ self.T(state[j + 1] ^ state[j + 2] ^ state[j + 3] ^ self.RK[31-j])
        for k in range(4):
            Out[k] = state[35 - k]
        return Out
